...
def binary_search_year(dates, year):
    dates.sort()
    #print("First entry")
    #print (f"My sorted array is {dates} \n")
    
    midDate = dates[len(dates)//2]
    #print (f"My midDate is {midDate}\n")
    
    if year == midDate.year:
        #print(f"Match found return True\n")
        return True
    elif year > midDate.year:
        start_index = dates.index(midDate)
        end_index = len(dates)
        if end_index == 0:
            #print(f"I am returning False from greater check\n")
            return False
        greater_arr = dates[start_index:end_index]
        #print(f"greater function {start_index} {end_index}\n")
        #print(f"My greater array is {greater_arr}\n")
        #print("Calling again from greater")
        return binary_search_year(greater_arr, year)
    else:
        start_index = 0
        end_index = dates.index(midDate)
        if end_index == 1:
            #print(f"I am returning False from lesser check\n")
            return False
        lesser_arr = dates[start_index:end_index+1]
        #print(f"lesser function {start_index} {end_index}\n")
        #print(f"My lesser array is {lesser_arr}\n")
        #print("Calling again from lesser")
        return binary_search_year(lesser_arr, year)
