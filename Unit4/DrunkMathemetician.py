...
def to_metric(string):
    ...
    from_number = float(original[0])
    from_unit = original[1]
    
    
    if from_unit == "gallons":
        return round(from_number * 3785.41, 2)
    
    if from_unit == "quarts":
        return round(from_number * 946.35, 2)
    
    if from_unit == "pints":
        return round(from_number * 473.18, 2)
    
    if from_unit == "cups":
        return round(from_number * 240, 2)
    
    if from_unit == "ounces":
        return round(from_number * 29.57, 2)
    
    if from_unit == "tablespoons":
        return round(from_number * 14.79, 2)
    
    if from_unit == "teaspoons":
        return round(from_number * 4.93, 2)
